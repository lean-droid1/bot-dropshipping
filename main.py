...
for k, v in os.environ.items():
    ...
    if "CLIENT_ID" in k: CLIENT_ID = v.strip()
    if "CLIENT_SECRET" in k: CLIENT_SECRET = v.strip()
...
def buscar_id_por_nombre_api(token, nombre_buscado):
    ...
    try:
        ...
        print(f"❌ Error buscando ID del producto: {e}")
        return None
...
def modificar_stock_api(token, producto_id, nuevo_stock):
    ...
    try:
        url_put = f"{URL_API_PRODUCTS}/{producto_id}"
...
def verificar_pedido_contra_proveedor(pedido, prod_proveedor):
    ...
    for item in pedido["productos"]:
        ...
        if encontrado and datos_prov:
            reporte += f"✅ *{item['nombre']}* (Cant: {item['cantidad']})\n• Proveedor: *CON STOCK* disponible a ${datos_prov['precio']:,}\n\n"
        else:
            todo_ok = False
