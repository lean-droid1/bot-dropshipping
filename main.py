...
MARGEN         = 0.78    # precio_web = costo_proveedor / 0.78  →  22% ganancia neta
SPAM_HORAS     = 6       # No repetir notificación de sin-stock por X horas
...
def marcar_sin_stock(producto_api):
    """Marca todas las variantes (o el producto) como sin stock."""
    pid       = producto_api["id"]
    variantes = producto_api["variantes"]
    if variantes:
        ok = True
        ...
        return set_visibilidad(pid, False)
...
def run_sync_total():
    ...
    for i, prod in enumerate(catalogo):
        ...
        if datos_prov is None:
            ...
            if ok:
                sinc[nombre_real] = {"sin_stock": True, "ts": time.time()}
...
def ciclo_monitoreo():
    ...
    for prod in catalogo:
        ...
        if datos_prov is None:
            # Proveedor no tiene → sin stock (anti-spam: solo cada SPAM_HORAS)
            ts_anterior = sync_actual.get("sin_stock_ts", 0)
            ya_marcado  = sync_actual.get("sin_stock", False)
            if not ya_marcado or (time.time() - ts_anterior) > SPAM_HORAS * 3600:
                if _token and marcar_sin_stock(prod):
                    sinc[nombre_real] = {"sin_stock": True, "sin_stock_ts": time.time()}
            ...
            if ok:
                ...
                sinc[nombre_real] = {"precio": p_min, "ts": time.time()}
                if int(precio_web) != p_min and p_min != ultimo_sinc:
